...
<html>
  ...
  <? include dirname(__FILE__) . "/config/header.php"; ?>
	...
	<tr><td><? echo _("Name"); ?>:</td><td><input type="text" size="25" name="realname" value="<? print $row['realname']; ?>" class="textfield"></td></tr>
	<tr><td><? echo _("Email Address"); ?>:</td><td><? print $row['username']; ?></td></tr>
	...
	<? if ($postmasteruidgid == "yes") {
	  ...
	  if ($domrow['quotas'] > "0") {
	    print "<tr><td>" . _("Mailbox quota") . " ({$domrow['quotas']} Mb max):</td>";
	...
	<tr><td><? echo _("Admin"); ?>:</td><td><input name="admin" type="checkbox" <?
	...
	?>
	<tr><td><? echo _("Enabled"); ?>:</td><td><input name="enabled" type="checkbox" <?
		if ($row['enabled'] == 1) { print "checked"; } ?> class="textfield"></td></tr>
	<tr><td><? echo _("Vacation on"); ?>:</td><td><input name="on_vacation" type="checkbox" <?
		if ($row['on_vacation'] == "1") { print " checked "; } ?> ></td></tr>
	<tr><td><? echo _("Vacation message"); ?>:</td>
	<td><textarea name="vacation" cols="40" rows="5" class="textfield"><? print $row['vacation']; ?></textarea>
	<tr><td><? echo _("Forwarding on"); ?>:</td><td><input name="on_forward" type="checkbox" <?
		if ($row['on_forward'] == "1") { print " checked "; } ?> ></td></tr>
	<tr><td><? echo _("Forward mail to"); ?>:</td>
	...
	<tr><td colspan="2" style="padding-top:1em"><? echo _("Aliases to this account "); ?>:<br>
	<?
    ...
    <table align="center">
	  ...
	  <tr><td><input name="submit" type="submit" value="<? echo _("Submit"); ?>"></td></tr>
      </form>
    </table>
    <table align="center">
      <tr><th><? echo _("Delete"); ?></th><th><? echo _("Blocked Header"); ?></th><th><? echo _("Content"); ?></th></tr>
      <? if ($blockresult->numRows()) { while ($blockrow = $blockresult->fetchRow()) {
	      print "<tr><td><a href=\"adminuserblocksubmit.php?action=delete&user_id={$_GET['user_id']}&block_id={$blockrow['block_id']}&localpart={$blockrow['localpart']}\"><img style=\"border:0;width:10px;height:16px\" title=\"Delete\" src=\"images/trashcan.gif\" alt=\"trashcan\"></a></td>";
