...
<html>
  ...
  <body onLoad="document.adminadd.realname.focus()">
    ...
    <form name="adminadd" method="post" action="adminuseraddsubmit.php">
      <table align="center">
	<tr><td><?php echo _("Name"); ?>:</td><td><input type="textfield" size="25" name="realname" class="textfield"></td></tr>
	<tr><td><?php echo _("Address"); ?>:</td><td colspan='2'><input type="textfield" size="25" name="localpart" class="textfield">@<?php print $_SESSION['domain']; ?></td></tr>
	...
	<?php if ($postmasteruidgid == "yes") {
	  print "<tr><td>" . _("UID") . ":</td><td><input type=\"textfield\" size=\"5\" name=\"uid\" class=\"textfield\" value=\"{$row['uid']}\"></td></tr>\n";
	  print "<tr><td>" . _("GID") . ":</td><td><input type=\"textfield\" size=\"5\" name=\"gid\" class=\"textfield\" value=\"{$row['gid']}\"></td></tr>\n"; 
	}
	if ($row['quotas'] > "0") {
	  print   "<tr><td>";
      printf (_("Mailbox quota (%s Mb max)"), $row['quotas']);
      print   ":</td>";
	  print "<td><input type=\"text\" size=\"5\" name=\"quota\" value=\"{$row['quotas']}\" class=\"textfield\">" . _("Mb") . "</td></tr>\n";
	} ?>
	<tr><td><?php echo _("Has domain admin privileges?"); ?></td><td><input name="admin" type="checkbox"></td></tr>
	<?php if ($row['pipe'] == "1") {
	...
	?>
	<tr><td><?php echo _("Enabled"); ?>:</td><td><input name="enabled" type="checkbox" checked></td></tr>
	<tr><td colspan="2" class="button"><input name="submit" type="submit" value="<?php echo _("Submit"); ?>"></td>
      </table>
    </form>
  </body>
</html>
