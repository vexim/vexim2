    ...
    <form name="adminadd" method="post" action="adminuseraddsubmit.php">
        <table align="center">
            ...
            <tr>
                <td></td>
                <td colspan="2">
                    <input type="button" value="<?php echo _('Generate password'); ?>"
                           onclick="suggestPassword('suggest')">
                    <input type="text" size="15" id="suggest" class="textfield">
                    <input type="button" value="<?php echo _('Copy'); ?>"
                           onclick="copyPassword('suggest', 'clear', 'vclear')">
                </td>
            </tr>
            <?php
            if ($postmasteruidgid == "yes") { ?>
                <tr>
                    ...
                    <td colspan="2"><input type="textfield" size="5" name="uid" class="textfield"
                                           value="<?php echo $row['uid']; ?>"></td>
                </tr>
                <tr>
                    <td><?php echo _('GID'); ?></td>
                    <td colspan="2"><input type="textfield" size="5" name="gid" class="textfield"
                                           value="<?php echo $row['gid']; ?>"></td>
                </tr>
            <?php }
            if ($row['quotas'] > "0") { ?>
                <tr>
                    <td><?php printf(_('Mailbox quota (%s Mb max)'), $row['quotas']); ?></td>
                    <td colspan="2"><input type="text" size="5" name="quota" class="textfield"
                                           value="<?php echo $row['quotas']; ?>"><?php echo _('Mb'); ?></td>
                </tr>
            <?php } ?>
            <tr>
