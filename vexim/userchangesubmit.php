...
# Update the password, if the password was given
  if (validate_password($_POST['clear'], $_POST['vclear'])) {
    ...
    if (!DB::isError($result)) {
      ...
      header ("Location: userchange.php?updated&user_id={$_GET['user_id']}");
      die;
    } else {
      header ("Location: userchange.php?badpass&user_id={$_GET['user_id']}");
    }
  }
...
# Finally 'the rest' which is handled by the profile form
    ...
    if (!DB::isError($result)) {
      ...
      header ("Location: userchange.php?failed&user_id={$_GET['user_id']}");
    }
