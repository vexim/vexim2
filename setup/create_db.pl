...
sub add_siteadminuser {
	...
	`stty echo`;
	  ...
	  $dbh->do("INSERT INTO $databasename.users (domain_id, localpart, username, password, uid, gid, smtp, pop, realname, type, admin)
	  		VALUES ('1',
			'siteadmin',
			'siteadmin',
			'$crypted',
...
$DBuserfields = "user_id,domain_id,localpart,username,password,uid,gid,smtp,pop,type,admin,on_avscan,on_blocklist,on_complexpass,on_forward,on_piped,on_spamassassin,on_vacation,enabled,flags,forward,maxmsgsize,quota,realname,sa_tag,sa_refuse,tagline,vacation";
