...
$mydbh->do("CREATE TABLE IF NOT EXISTS domains (domain_id mediumint(8) unsigned NOT NULL
auto_increment,
...
$mydbh->do("CREATE TABLE IF NOT EXISTS users (user_id int(10) unsigned NOT NULL auto_increment,
        ...
        type enum('local','alias','catch', 'fail', 'piped', 'admin') NOT NULL default 'local',
...
$pgdbh->do("CREATE TABLE domains (domain_id INT PRIMARY KEY DEFAULT NEXTVAL('serial'),
...
$pgdbh->do("CREATE TABLE users (user_id INT PRIMARY KEY DEFAULT NEXTVAL('serial'),
          ...
          pop varchar(255) NOT NULL default '',
          realname varchar(255) NOT NULL default '',
          type varchar(8) CHECK(type in ('local','alias','catch', 'fail', 'piped', 'admin')) NOT NULL,
          ...
          vacation varchar(255) default NULL,
          flags varchar(16) default NULL,
          tagline varchar(255) default NULL)") or die "Could not create table users";
...
while (my $ref = $sth->fetchrow_hashref()) {
  ...
  my $lkp = $mydbh->prepare("INSERT INTO vexim.users (domain_id, localpart, clear, crypt, uid, gid, smtp, pop, realname, type, admin)
...
my $sth = $mydbh->prepare("UPDATE vexim.users SET localpart='siteadmin' and type='admin' WHERE localpart='site' and realname='SiteAdmin'");
$sth->execute();

print "Exporting data from MySQL...\n";
my $sth = $mydbh->prepare("select * INTO OUTFILE '/tmp/vexim-mysql-migrate' FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY \"'\" LINES TERMINATED BY \"\n\" from vexim.users");
$sth->execute();

open IN, "</tmp/vexim-mysql-migrate" or die $!;
...
while(<IN>)
{
  ...
  #print OUT;
  #print;
  my $sth = $pgdbh->prepare("INSERT INTO users VALUES ($_)");
  $sth->execute();
}
close IN;
print "Migration complete!\n";
print "Please delete /tmp/vexim-mysql-migrate\n\n";
